...
tDCSdata = tDCSdata[tDCSdata$M1=="tms",]
...
trEffect_pvals = sapply(1:dim(trEffect)[2], function(x)
  round(coef(trEffect[, x])["(Intercept)", "Pr(>|t|)"], 3))
...
pvals = sapply(1:dim(lmPost)[2], function(x)
  coef(lmPost[, x])["stimulationactive", "Pr(>|t|)"])
...
write.table(results, file = "T:/_CogNeurPhD/03-Experiment_tDCS/resultsInterceptsExclC4.txt")

formattedresults = print(xtable(results), type="html")
write.table(formattedresults, file = "T:/_CogNeurPhD/03-Experiment_tDCS/resultsInterceptsExclC4.html")
...
# export to csv
#write.csv(tDCSdata, file = "T:/_CogNeurPhD/03-Experiment_tDCS/tDCSdataWithITPostDiffPred.csv")
